...
def send_log(opt_code, rid, user_login):
    ...
    if cache_key in sent_logs_cache:
        logger.info(
            f"Log with RID {rid} and OPT_CODE {opt_code} has already been sent recently."
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"Log with RID {rid} and OPT_CODE {opt_code} sent successfully."
            )
        else:
            logger.error(
                f"Failed to send log with RID {rid} and OPT_CODE {opt_code}: {response.status_code}"
            )
...
# 获取 token 的函数
def get_token():
    data = {"UserLogin": "LZ01", "Password": "123456"}
