...
class ModbusClientSingleton:
    ...
    def _get_conn(self):
        try:
            ...
            logger.error(f"Modbus connection {self._client.comm_params.host}:{self._client.comm_params.port} {exc}")
        finally:
            self._client.close()
    ...
    def modbus_conn(func):
        ...
        def wrapper(self, *args, **kw):
            ...
            raise ConnectionError("modbus_conn context initialization failed")
        return wrapper

    @modbus_conn
    def modbus_test_conn(self):
    ...
    def write_ascii_string(self, register_address: int, input_string: str):
        ...
        for i in range(0, len(input_string), 2):
            ...
            low_byte = ord(input_string[i + 1]) if i + \
                1 < len(input_string) else 0  # 低字节
        ...
        try:
            result = self._client.write_registers(
                register_address, registers, slave=1)
            if result.isError():
    ...
    def read_float_test(self, start_address, end_address):
        ...
        for i in range(start_address, end_address + 1, max_registers_per_request):
            ...
            current_batch_end = min(
                i + max_registers_per_request - 1, end_address)
            num_registers_batch = current_batch_end - i + 1

            # 打印当前读取的寄存器范围
            print(
                f"正在读取寄存器：起始地址 {i}，结束地址 {current_batch_end}，数量 {num_registers_batch}")

            # 读取寄存器的值
            result = self._client.read_holding_registers(
                i, count=num_registers_batch, slave=1)
            ...
            for j in range(0, num_registers_batch, 2):
                byte_data = struct.pack(
                    # 大端字节序
                    '>HH', result.registers[j + 1], result.registers[j])
    ...
    def read_float_ASCII(self, address_from: int, address_to: int):
        ...
        result = self._client.read_holding_registers(
            address_from, count=length, slave=1)
    ...
    def write_ASCII_string_to_registers(self, address_from, address_to, ascii_string):
        ...
        for i in range(0, len(swapped_bytes), 2):
            register = (swapped_bytes[i] << 8) + \
                swapped_bytes[i + 1]  # 组合成寄存器值
        ...
        try:
            result = self._client.write_registers(
                address_from, registers, slave=1)
            if result.isError():
                logging.error("写入寄存器数据时出错")
                return False
            return True
