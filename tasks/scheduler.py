...
def process_solders(mode,session):
    """
    左连接查询 Solder 和 SolderModel 表，筛选 OrderDateTime 不为空的数据，
    计算时间差，执行 modbus_write 操作，并去除已操作的 solder。

    参数：
    - session: 数据库会话对象
    - modbus_client: Modbus 客户端对象
    ...
    try:
        if mode==100:
            ...
            # 左连接查询 Solder 和 SolderModel 表，并筛选 OrderDateTime 不为空的数据
            solder_models = session.query(Solder, SolderModel).join(
                SolderModel, Solder.Model == SolderModel.Model, isouter=True
            ).filter(Solder.OrderDateTime != None).all()
            processed_solder_codes = set()  # 存储已操作过的 SolderCode

            # 遍历 solder_models 数据，计算时间差并执行 modbus_write 操作
            for item, model in solder_models:
                order_time = item.OrderDateTime
                sitr_time = timedelta(seconds=model.StirTime)
                rewarm_time = timedelta(minutes=model.RewarmTime)
                ...
                if target_time <= current_time:
                    # 执行 modbus_write 操作
                    modbus_client.modbus_write("jcq", 2, item.StationID, 1)
                    processed_solder_codes.add(item.SolderCode)  # 记录已操作的 SolderCode

            # 在 solders 列表中去除已操作的 solder
            prepared_solders = [solder for solder in solder_models if solder.SolderCode in processed_solder_codes]

            session.commit()
            return prepared_solders
        # elif mode == 0:
        #     return []

    except Exception as e:
        session.rollback()
        logger.error(f"预约判定操作失败: {str(e)}")
        return []
    except TimeoutError:
...
def move_update(mode):
    ...
    # 查询 Solder 表中所有的 Model 和对应的 StationID
    # solder_data = (
    #     session.query(Solder.Model, Solder.StationID)
    #     .filter(Solder.StationID.between(601, 840))  # 合并两个区间的查询
    #     .all()
    # )
    # 查询所有非预约的 Solder 数据
    # solders=process_solders(mode,session)
    solders = session.query(Solder).all()
    ...
    # 提取所需字段（Model 和 StationID），并确保 StationID 转换为整数
    solder_data = [
    ...
    for solder in solder_data:  # 遍历列表中的字典
        ...
        if 601 <= station_id <= 660:
            count_by_model[model]["count_601_660"] += 1
        elif 801 <= station_id <= 840:
            count_by_model[model]["count_801_840"] += 1
    ...
    try:
        ...
        fang_601_660 = 1 if 601 <= solder_fang_station_cache <= 660 else 0
        fang_801_840 = 1 if 801 <= solder_fang_station_cache <= 840 else 0
        ...
        #
            ...
            if start == 201:
                for solder, model_data in solder_data:
                    if int(solder.StationID) in timeout_cache:
                        continue
                    if model_data:
                        amount = model_data.RewarmNum - count_by_model[model_data.Model]["count_601_660"] - fang_601_660
            ...
            # # 查询 Station 表中符合条件的数据
            # # station_data = session.query(Station).filter(
            # #     Station.StationID >= start,
            # #     Station.StationID <= end,
            # #     ~Station.StationID.in_(solder_station_ids)
            # # ).all()
            # # 假设 stations 是已经查询的所有 Station 数据
            ...
            for model, counts in count_by_model.items():
                ...
                if solder_model:
                    ...
                    logger.info(
                        f"查询结果: count_801_840={counts['count_801_840']}, count_601_660={counts['count_601_660']}")

                    # 根据查询结果设置 huiwen_daiqu 的值
                    huiwen_daiqu = counts["count_801_840"]+fang_801_840 < ready_out_num
                    logger.info(f"设置 huiwen_daiqu = {huiwen_daiqu} for Model {model}")

                    # 根据查询结果设置 lengcang_huiwen 的值
                    lengcang_huiwen = counts["count_601_660"]+fang_601_660 < rewarm_num
            ...
            if 601 == start:
                # 对于 StationID 在 601 到 660 范围内的记录
                if lengcang_huiwen == False:
                    # 当 lengcang_huiwen == False 时，遍历 601 到 660 范围内的所有数字
                    for station_id in range(601, 661):  # 范围 601 到 660
            ...
            if 801 == start:
                if huiwen_daiqu == False:
                    # 当 lengcang_huiwen == False 时，遍历 601 到 660 范围内的所有数字
                    for station_id in range(801, 841):  # 范围 601 到 660
                        if station_id not in solder_station_ids and station_id != solder_fang_station_cache:
                            modbus_client.modbus_write("jcq", 0, station_id, 1)
                else:
                    for station_id in range(801, 841):  # 范围 801 到 840
                        if station_id not in solder_station_ids and station_id != solder_fang_station_cache:
                            modbus_client.modbus_write("jcq", 1, station_id, 1)
            if 201 == start :
