...
def task_freeze():
    with db_instance.get_session() as db_session:
        ...
        solder_stationID_list_cold   = [r.StationID for r in db_session.query(Solder.StationID).filter(Solder.StationID.between(  ADDR_REGION_COLD_START,   ADDR_REGION_COLD_END)).all()]
        solder_stationID_list_rewarm = [r.StationID for r in db_session.query(Solder.StationID).filter(Solder.StationID.between(ADDR_REGION_REWARM_START, ADDR_REGION_REWARM_END)).all()]
        solder_stationID_list_wait   = [r.StationID for r in db_session.query(Solder.StationID).filter(Solder.StationID.between(  ADDR_REGION_WAIT_START,   ADDR_REGION_WAIT_END)).all()]
