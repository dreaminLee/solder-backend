...
def add_alarm():
    with db_instance.get_session() as session:
        ...
        # 创建 Alarm 实例
        new_alarm = Alarm(
            AlarmText=alarm_text, StartTime=alarm_datetime, EndTime=None, Kind=kind
        )
...
def get_alarms():
    ...
    with db_instance.get_session() as session:
        ...
        alarms_query = (
            session.query(Alarm)
            .filter(and_(Alarm.StartTime >= start_date, Alarm.StartTime <= end_date))
...
def clear_alarms():
    with db_instance.get_session() as session:
        ...
        session.commit()
        return jsonify(
            Response.SUCCESS({"message": f"{num_of_rows_deleted}条报警信息已清空"})
        )
