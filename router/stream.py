    ...
    def generate_sse_data():
        ...
        while True:
            try:
                ...
                session.close()
                # 判断数据是否变化
                if (cold_storage_count != last_cold_storage_count or
                        warm_storage_count != last_warm_storage_count or
                        waiting_pickup_count != last_waiting_pickup_count):
                    # 组装 SSE 数据
                    data = {
    ...
    def generate_sse_data():
        ...
        while True:
            try:
                ...
                changed_to_true = {k: v for k, v in alarm_data.items() if previous_alarm_data.get(k) == 0 and v == 1}

                # 找出从 True 到 False 的变化（结束报警）
                changed_to_false = {k: v for k, v in alarm_data.items() if previous_alarm_data.get(k) == 1 and v == 0}

                # 如果有从 False 到 True 的变化，处理为 Type="start"
                if changed_to_true:
                    thread = Thread(target=background_db_task, args=(changed_to_true, "start"))
                    thread.start()

                # 如果有从 True 到 False 的变化，处理为 Type="end"
                if changed_to_false:
                    thread = Thread(target=background_db_task, args=(changed_to_false, "end"))
                    thread.start()
        ...
        try:
            process = subprocess.Popen(
                [
                    ...
                    '-i', rtsp_url,  # RTSP 输入地址
                    '-c:v', 'libx264',  # 使用 H.264 编解码器
                    '-preset', 'ultrafast',  # 编码速度设置为最快
                    '-f', 'mp4',  # 输出格式为 MP4
                    '-tune', 'zerolatency',  # 优化为低延迟模式
                    '-b:v', '1M',  # 设置视频比特率
                    '-an',  # 禁用音频
