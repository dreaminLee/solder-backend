...
def get_solders():
    ...
    try:
        ...
        if model != "" and StaArea != "":
            ...
            solders = (
                session.query(Solder, Station.StaArea)
                .filter(Station.StaArea == StaArea, Solder.Model == model)
                .join(Station, Solder.StationID == Station.StationID)
                .all()
            )
        elif model != "":
            ...
            solders = (
                session.query(Solder, Station.StaArea)
                .filter(Station.StaArea == StaArea)
                .join(Station, Solder.StationID == Station.StationID)
                .all()
            )
        else:
...
def unordered_solder():
    with db_instance.get_session() as db_session:
        solders_unordered = (
            db_session.query(Solder, SolderModel.MinLcTime)
        ...
        return Response.SUCCESS(
            [
                ...
                if datetime.now()
                >= solder.StorageDateTime + timedelta(hours=min_lc_time)
            ]
        )
...
def order_solder():
    ...
    with db_instance.get_session() as session:
        ...
        solders_unordered = (
            session.query(Solder, SolderModel.MinLcTime)
            .join(SolderModel, SolderModel.Model == Solder.Model)
            .filter(
                Solder.StationID.between(ADDR_REGION_START_COLD, ADDR_REGION_END_COLD),
                Solder.OrderUser == None,
            ...
            .order_by(desc(Solder.BackLCTimes))
            .all()
        )
...
def daiqu_solder():
    with db_instance.get_session() as db_session:
        solders_in_rewarm_wait_model = (
            db_session.query(Solder, SolderModel)
            .join(SolderModel, SolderModel.Model == Solder.Model)
            .filter(
                or_(
                    Solder.StationID.between(
                        ADDR_REGION_START_REWARM, ADDR_REGION_END_REWARM
                    ),
                    Solder.StationID.between(
                        ADDR_REGION_START_WAIT, ADDR_REGION_END_WAIT
                    ),
                )
            )
            .all()
        ...
        solders_outable = [
            solder
            for solder, solder_model in solders_in_rewarm_wait_model
            if (
                solder_model.JiaobanRule == "自动搅拌"
                and in_region_wait(solder.StationID)
                and modbus_client.modbus_read("jcq", solder.StationID, 1)[0] == 0
            )
            or (
                solder_model.JiaobanRule == "出库搅拌"
                and in_region_rewarm(solder.StationID)
                and modbus_client.modbus_read("jcq", solder.StationID, 1)[0] == 0
                and (
                    datetime.now() >= solder.RewarmEndDateTime
                    or solder.OrderUser != None
                )
            )
        ]
        return Response.SUCCESS(
            [
                {
                    ...
                    "StorageUser": solder.StorageUser,
                    "StorageDateTime": (
                        solder.StorageDateTime.strftime("%Y-%m-%d %H:%M:%S")
                        if solder.StorageDateTime
                        else None
                    ),
...
def accessible_solder():
    with db_instance.get_session() as db_session:
        solders_in_rewarm_wait = (
            db_session.query(Solder)
            .filter(
                or_(
                    Solder.StationID.between(
                        ADDR_REGION_START_REWARM, ADDR_REGION_END_REWARM
                ...
                for res in (
                    {
                        ...
                        "Station": solder.StationID,
                        "Station_status": modbus_client.modbus_read(
                            "jcq", solder.StationID, 1
                        )[0],
                    }
                    for solder in solders_in_rewarm_wait
                )
                if (in_region_wait(res["Station"]) and res["Station_status"] == 2)
                or (in_region_rewarm(res["Station"]) and res["Station_status"] == 22)
            ]
        )
...
def out_solder():
    ...
    with db_instance.get_session() as db_session:
        ...
        solders_model = (
            db_session.query(Solder, SolderModel)
            .join(SolderModel, SolderModel.Model == Solder.Model)
            .filter(Solder.Model == model_type)
            .all()
        )
        ...
        if len(solders_outable) < amount:
            return Response.FAIL(
                f"未找到足够的锡膏记录，当前只有 {len(solders_outable)} 条"
            )
        ...
        for solder, rule in solders_outable[:amount]:
            ...
            modbus_client.modbus_write(
                "jcq", 2 if rule == "自动搅拌" else 22, solder.StationID, 1
            )
    ...
    records = [
        {
            ...
            "StorageDateTime": (
                record.StorageDateTime.strftime("%Y-%m-%d %H:%M:%S")
                if record.StorageDateTime
                else None
            ),
        }
