...
def start_periodic_insertion():
    ...
    # 确保只启动一个线程
    if (
        periodic_insertion_thread is None or not periodic_insertion_thread.is_alive()
    ):  # 如果线程未运行
        periodic_insertion_thread = threading.Thread(target=periodic_insertion)
        periodic_insertion_thread.daemon = (
            True  # 设置为守护线程，确保程序退出时自动停止
        )
...
def get_temperature_records():
    ...
    try:
        ...
        # 如果没有找到记录
        if not records:
            return Response.SUCCESS(data={"list": [], "total": 0})
        ...
        return Response.SUCCESS(data={"list": records_data, "total": total})
