...
# 输入字符串列表
commands = [
    "三色灯黄", "三色灯绿", "三色灯红", "三色灯蜂鸣",
    "入库门打开", "出库门打开", "出库滑台动作", "出库滑台复位",
    "冰箱门推拉动作", "冰箱门推拉复位", "冰箱门滑台动作", "冰箱门滑台复位",
    "模组夹爪气缸动作", "模组夹爪气缸复位", "模组180度旋转气缸动作", "模组180度旋转气缸复位",
    "模组45度旋转气缸动作", "模组45度旋转气缸复位", "扫码旋转电机动作", "搅拌电机动作" ,'报警复位按钮', '回原位按钮','一键冷藏按钮'
]
...
def set_sport():
    ...
    if tag == 1:
        ...
        # 立即执行 tag=1 的操作
        write_value = [True]
        coil_status = modbus_client.modbus_write("xq", write_value, int(c_a_dict[action]), 1)
...
def handle_auto_tag2(action):
    ...
    with lock:
        ...
        if pending_tag2 and (time.time() - last_tag1_time >= 1):
            pending_tag2 = False
            write_value = [False]
            coil_status = modbus_client.modbus_write("xq", write_value, c_a_dict[action], 1)
...
# 假设这是你定义的 tuned_dict
tuned_dict = {
    'X轴点动向左': 540, 'X轴绝对定位': 541, 'Y轴点动向前': 542, 'Y轴绝对定位': 543,
    'Z轴点动向上': 544, 'Z轴绝对定位': 545, 'R轴点动正转': 546, 'R轴绝对定位': 547,
    '搅拌电机转动一组': 548, '扫码旋转转动一组': 549, 'X轴原点位置确认': 550, 'Y轴原点位置确认': 551,
    'Z轴原点位置确认': 552, 'R轴回原点': 553, '搅拌电机回原点': 554, 'X轴点动向右': 555,
    'Y轴点动向后': 556, 'Z轴点动向下': 557, 'R轴点动反转': 558, '手动xy45度动作向上': 562, '手动xy45度动作向下': 563
}
...
def tuned_operate():
    ...
    with lock:
        ...
        # 写线圈
        result2 = modbus_client.modbus_write("xq", write_value, int(tuned_dict[action]), 1)
...
def tuned_speed():
    tuned_dict = {
        'XYZR手动速度百分比': 1512, 'X轴满载速度': 1514, 'Y轴满载速度': 1516,'Z轴满载速度':1518,'R轴满载速度':1520,'XYZR自动速度百分比':1538
    }
    data = request.get_json()
