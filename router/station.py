...
def get_all_stations():
    ...
    try:
        ...
        stations = (
            ...
            .filter(Station.SolderCode >= 0)
            .order_by(Station.SolderCode)
            .all()
        )
        ...
        def modbus_read_disabled_bulk(station_disabled_list):
            ...
            for station_disabled in station_disabled_list:
                disabled_data[station_disabled] = modbus_client.modbus_read(
                    "jcq", station_disabled, 1
                )
                ...
                station_list.append(
                    {
                        ...
                        "ModifyDateTime": (
                            station.ModifyDateTime.strftime("%Y-%m-%d %H:%M:%S")
                            if station.ModifyDateTime
                            else None
                        ),
                    }
                )
        ...
        def process_stations_parallel(stations, float_values):
            ...
            # 批量处理 Disabled 状态
            station_disabled_list = [
                station.Disabled for station in stations if station.Disabled
            ]
            ...
            with ThreadPoolExecutor() as executor:
                results = executor.submit(
                    process_station_batch, stations, float_values, index, disabled_data
                )
    ...
    with db_instance.get_session() as session:
        return Response.SUCCESS(
            data=[
                ...
                for station in session.query()
                .with_entities(Station.StaArea, Station.StationID, Station.StaType)
                .all()
            ]
        )
